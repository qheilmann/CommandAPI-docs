...
function resolveValue(value: { default: any; vId?: number }): string {
  if (value?.vId && valueMap[value.vId]) {
    return valueMap[value.vId];
  }
  value.vId = valueMap.length;
  if (value.default.toString().startsWith("$")) {
    return (valueMap[value.vId] = dynamicValue(value.default.toString()));
  } else {
    return (valueMap[value.vId] = value.default.toString());
  }
}
      ...
      >
        ...
        >
          ...
          <span
            ...
            :class="
              value.default.toString().startsWith('$')
                ? 'config-value-random'
